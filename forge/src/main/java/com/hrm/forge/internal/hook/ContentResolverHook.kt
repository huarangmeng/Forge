...
internal object ContentResolverHook {
    ...
    fun hook(context: Context) {
        if (isHooked) {
            Logger.d(TAG, "ContentResolver already hooked, skip")
            return
        }
        
        try {
            Logger.i(TAG, "Start hooking ContentResolver...")
            
            val contentResolver = context.contentResolver
            
            // Hook IContentService
            hookIContentService(contentResolver)
            
            isHooked = true
            Logger.i(TAG, "✅ ContentResolver hooked successfully")
            
        } catch (e: Exception) {
            Logger.e(TAG, "Failed to hook ContentResolver", e)
        }
    }
     ...
     */
    private fun hookIContentService(contentResolver: ContentResolver) {
        try {
            ...
            if (originalService == null) {
                ...
                try {
                    val getContentServiceMethod = contentResolverClass.getDeclaredMethod("getContentService")
    ...
    fun reset() {
        isHooked = false
        Logger.d(TAG, "ContentResolverHook reset")
    }
}
