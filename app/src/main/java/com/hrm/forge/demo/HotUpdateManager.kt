...
class HotUpdateManager(val context: Context) {
     ...
     * @param callback 回调 (result, message)
     */
    fun releaseNewVersion(
        apkFilePath: String,
        callback: (ReleaseResult, String) -> Unit
    ) {
        CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                if (!apkFile.exists()) {
                    ...
                    Log.e(TAG, msg)
                    callback(ReleaseResult.APK_NOT_FOUND, msg)
                    return@launch
                }
                ...
                if (result.isSuccess) {
                    ...
                    val msg = "Release failed: ${result.message} (code: ${result.code})"
                    Log.e(TAG, msg)
                    callback(result, msg)
                }
            } catch (e: Exception) {
                ...
                Log.e(TAG, msg, e)
                callback(ReleaseResult.UNKNOWN_ERROR, msg)
            }
        }
    }
