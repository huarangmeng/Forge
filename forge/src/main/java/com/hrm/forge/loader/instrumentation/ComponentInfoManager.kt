...
object ComponentInfoManager {
    ...
    fun init(context: Context, hotUpdateApkPath: String?) {
        if (isInitialized) {
            Logger.i(TAG, "ComponentInfoManager already initialized, skip")
            return
        }
        ...
        try {
            ...
            isInitialized = true
            Logger.i(TAG, "✅ ComponentInfoManager initialized in ${elapsedTime}ms")
    ...
    fun clear() {
        ...
        isInitialized = false
        Logger.i(TAG, "ComponentInfoManager cleared")
    }
    ...
    fun getStats(): String {
        return buildString {
            appendLine("ComponentInfoManager Stats:")
