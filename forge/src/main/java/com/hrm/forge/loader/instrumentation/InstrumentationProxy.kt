...
class InstrumentationProxy(private val base: Instrumentation) : Instrumentation() {
     ...
     * 2. 如果未注册，替换为占坑 Activity
     * 3. 保存真实 Activity 信息，在 newActivity 时恢复
     */
    ...
    fun execStartActivity(
        ...
        requestCode: Int,
        options: Bundle?
    ): Instrumentation.ActivityResult? {  // 返回值可能为 null
        ...
        val result = normalStartActivity(who, contextThread, token, target, intent, requestCode, options)
    ...
    private fun normalStartActivity(
        ...
        return try {
            execMethod.invoke(
                ...
                requestCode,
                options
            ) as? Instrumentation.ActivityResult  // 使用安全转换，允许 null
